...
stdenv.mkDerivation (finalAttrs: {
  ...
  env = {
    ...
    CXXFLAGS = cFlags;
    ROCM_PATH = clr;
    LD_LIBRARY_PATH = lib.makeLibraryPath [ rocm-runtime ];
  };
  ...
  nativeBuildInputs = [
    ...
    rocm-cmake
    clr
    # 7.13 added a find_package(Python3 REQUIRED) at configure time
    # (used by the KDB validation scripts).
    python3
  ];
  ...
  postPatch = ''
    ...
    # 7.13 dropped src/composable_kernel/ (it's no longer vendored —
    # CK is now a build dep) so patchShebangs would fail on it.
    patchShebangs test fin utils install_deps.cmake

    # 7.13 also stopped hardcoding "/opt/rocm" in src/comgr.cpp; the
    # HIPRTC include path is read from $ROCM_PATH at runtime. We set
    # ROCM_PATH via the clr setup hook in dependent builds, and the
    # substitution is no longer applicable.
  ''
