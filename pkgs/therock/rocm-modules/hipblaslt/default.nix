...
let
  ...
  supportedTargets = lib.lists.intersectLists gpuTargets [
    "gfx908"
    "gfx90a"
    "gfx942"
    "gfx950"
  ...
  py = python3.withPackages (ps: [
    ...
    ps.nanobind
    ps.msgpack
    # 7.13's Tensile imports joblib for parallel codegen.
    ps.joblib
  ]);
...
stdenv.mkDerivation (finalAttrs: {
  ...
  env = {
    ...
    CXXFLAGS = cFlags;
    ROCM_PATH = "${clr}";
    TENSILE_ROCM_ASSEMBLER_PATH = lib.getExe' clr "amdclang++";
    TENSILE_GEN_ASSEMBLY_TOOLCHAIN = lib.getExe' clr "amdclang++";
  ...
  preConfigure = ''
    ...
    # Tensile (helper_cache, joblib cache dir) writes under $HOME during
    # TensileCreateLibrary; the Nix sandbox sets HOME=/homeless-shelter
    # which is unwritable.
    export HOME=$NIX_BUILD_TOP/tensile-home
    mkdir -p "$HOME"
  '';

  # 7.13 moved the Git find_package into the bundled `origami` shared
  # library (../../shared/origami/cmake/dependencies.cmake). We don't have
  # .git in the source slice, so drop that requirement.
  # Also link tensilelite-host against zstd — messagepack-compression-
  # support.patch adds ZSTD_isError/ZSTD_decompress calls to MessagePack.cpp
  # but does not extend target_link_libraries, so the symbol resolves only
  # when downstream consumers happen to pull zstd in transitively (not the
  # case for hipblaslt's clients-common archive).
  postPatch = ''
    substituteInPlace ../../shared/origami/cmake/dependencies.cmake \
      --replace-fail "find_package(Git REQUIRED)" ""
    substituteInPlace tensilelite/CMakeLists.txt \
      --replace-fail \
        'target_link_libraries(tensilelite-host PRIVATE ''${hipblaslt_msgpack_target})' \
        'target_link_libraries(tensilelite-host PRIVATE ''${hipblaslt_msgpack_target} ${lib.getLib zstd}/lib/libzstd.so)'
  '';
