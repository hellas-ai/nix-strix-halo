...
stdenv.mkDerivation (finalAttrs: {
  ...
  buildInputs = [
    ...
    openmp
    amd-blis
    # 7.13 introduced a hard amd_smi dependency where 7.2 had none.
    amdsmi
  ]
  ...
  env = {
    CXXFLAGS = "-fopenmp -I${lib.getDev boost}/include -I${hipblas-common}/include -I${roctracer}/include";
    # Fails to link tests with undefined symbol: cblas_*
    LDFLAGS = "-Wl,--as-needed -lzstd" + lib.optionalString (buildTests || buildBenchmarks) " -lcblas";
    TENSILE_ROCM_ASSEMBLER_PATH = "${stdenv.cc}/bin/clang++";
  };
  ...
  postInstall =
    ...
    + ''
      if [ -d $out/bin ]; then
        # 7.13 also installs *.xml regression manifests, a stray
        # rocblas/CTestTestfile.cmake CTest leftover, and the
        # rocblas_clients_readme.txt + rocblas_gtest.data fixtures into
        # $out/bin. Drop them all; none belong in the main runtime output.
        rm -f $out/bin/*.{yaml,txt,xml,data}
