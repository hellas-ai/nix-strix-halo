...
{ lib, writeShellScriptBin, tmux, amdgpu_top, btop, nvtopPackages, ec-su-axb35-monitor, systemd }:
...
writeShellScriptBin "strixtop" ''
  ...
  if [ $? != 0 ]; then
      ...
      ${tmuxExe} new-session -d -s "$SESSION_NAME" -n "monitoring" 'sudo ${amdgpuTopExe}'
      ...
      ${tmuxExe} send-keys -t "$SESSION_NAME:monitoring.3" 'sudo ${nvtopExe}' C-m
