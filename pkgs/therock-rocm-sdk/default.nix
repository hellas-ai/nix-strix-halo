  ...
  installPhase = ''
    ...
    patch_elf() {
      ...
      if old_rpath=$(patchelf --print-rpath "$elf" 2>/dev/null); then
        case ":$old_rpath:" in
          *":$gcc_runtime:"*) new_rpath="$old_rpath" ;;
          *) new_rpath="''${old_rpath:+$old_rpath:}$gcc_runtime" ;;
        esac
        case ":$new_rpath:" in
          *":$libdrm_runtime:"*) ;;
          *) new_rpath="''${new_rpath:+$new_rpath:}$libdrm_runtime" ;;
        esac
    ...
    for dir in "$out/bin" "$out/llvm/bin" "$out/lib/llvm/bin"; do
      ...
      for tool in \
        ...
        lld ld.lld llvm-config llvm-ar llvm-ranlib llvm-link; do
        [ -f "$dir/$tool" ] || continue
        patch_elf "$dir/$tool"
      done
    done
