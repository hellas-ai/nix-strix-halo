...
# tokenizers-cpp upstream doesn't commit Cargo.lock; the rev pin below
# must stay in sync with the vendored ./Cargo.lock. Bumping the rev
# without regenerating the lock will fail at the cargo vendor step.
let
  ...
  tokenizers-c = rustPlatform.buildRustPackage {
    ...
    inherit src;
    sourceRoot = "${src.name or "source"}/rust";
