...
stdenv.mkDerivation (finalAttrs: {
  ...
  # rocfft 7.13 bumped its SQLite minimum to 3.51.3 (CVE-only release).
  # nixpkgs is at 3.51.2 and the backup API rocfft actually uses has been
  # available since 3.36.0, so drop the version floor.
  postPatch = ''
    substituteInPlace cmake/sqlite.cmake \
      --replace-fail 'find_package(SQLite3 3.51.3 REQUIRED)' \
                     'find_package(SQLite3 3.51.2 REQUIRED)'
  '';
