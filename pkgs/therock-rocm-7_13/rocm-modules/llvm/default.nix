...
let
  ...
  llvmSrc =
    if therockSource != null then
      runCommand "therock-llvm-project-source-${version}" { } ''
        mkdir -p "$out"
        cp -a --reflink=auto ${therockSource}/compiler/amd-llvm/. "$out/"
        chmod -R u+w "$out"
      ''
    else
      fetchFromGitHub {
  ...
  commonCmakeFlags = [
    ...
    # PCH gets compiled with the project-level CXX flags but downstream TUs
    # also pick up the cc-wrapper hardening defaults (-O2 etc.), so clang
    # rejects the PCH with "OptimizationLevel differs". Disable PCH so the
    # compilations stay flag-consistent. Costs a few % of build wallclock,
    # which is fine for an aarch-stable LLVM that we cache.
  ...
  clang-unwrapped = (
    (s.prev.clang-unwrapped.override {
      ...
      (old: {
        ...
        patches =
          # The scan-build-py-configurable backport (PR #172727) is already
          # present in ROCm LLVM 23.0; only add it for the upstream LLVM 22
          # fallback path.
          lib.optionals (therockSource == null) [
            (fetchpatch {
          ...
          # clang-log-jobs.diff patches lib/Driver/Compilation.cpp around the
          # JobScheduler refactor that landed in LLVM 23; its second hunk no
          # longer applies on the ROCm 23.0 tree. Skip it on therockSource
          # since it's a logging-only quality-of-life patch.
          ++ lib.optionals (therockSource == null) [
    ...
    (llvmPackagesRocm.openmp.override {
      ...
      (
        old:
        {
          ...
          # LLVM 23 removed the standalone `openmp/` cmake entry point — it
          # now has to be built via the runtimes coordinator. Repackage the
          # source so it carries `runtimes/` alongside `cmake/` and `openmp/`,
          # then point sourceRoot at runtimes/ and set LLVM_ENABLE_RUNTIMES.
          # Only do this for the ROCm LLVM 23 path; the upstream LLVM 22
          # fallback still uses the standalone build.
        }
        // lib.optionalAttrs (therockSource != null) {
          # runtimes/CMakeLists.txt resolves AddLLVM via
          # ${CMAKE_CURRENT_SOURCE_DIR}/../llvm/cmake/modules, so we need a
          # sibling llvm/ tree with cmake/ and utils/llvm-lit. Likewise the
          # openmp unittests subdirectory unconditionally calls build_gtest()
          # which needs third-party/unittest. Copy only what's referenced
          # rather than all 1.4G of llvm/.
          src = runCommand "openmp-src-${version}" { } ''
            mkdir -p "$out/llvm/utils"
            cp -r ${llvmSrc}/cmake "$out/"
            cp -r ${llvmSrc}/openmp "$out/"
            cp -r ${llvmSrc}/runtimes "$out/"
            cp -r ${llvmSrc}/third-party "$out/"
            cp -r ${llvmSrc}/llvm/cmake "$out/llvm/"
            cp -r ${llvmSrc}/llvm/utils/llvm-lit "$out/llvm/utils/"
          '';
          sourceRoot = "openmp-src-${version}/runtimes";
          # openmp/runtime/CMakeLists.txt has custom_commands that write into
          # openmp/runtime/exports/ in the source tree at build time, so the
          # whole copy must be writable. Default unpackPhase only chmods +w
          # for the sourceRoot we cd into (runtimes/), not its siblings.
          postUnpack = ''
            chmod -R u+w "openmp-src-${version}"
          '';
        }
        // {
          cmakeFlags =
