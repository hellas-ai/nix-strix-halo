# NixOS module for FastFlowLM (FLM) on the AMD Ryzen AI NPU.
#
...
# `services.fastflowlm.modelsPath`.
{
  config,
  lib,
  pkgs,
...
in
{
  options.services.fastflowlm = {
    ...
    setIommuPt = mkOption {
      ...
      default = true;
      description = ''
        Add `iommu=pt` to the kernel command line. FastFlowLM's AMD XDNA
        runtime requires an enabled IOMMU, and ROCm/RCCL userspace also checks
        this exact x86 passthrough spelling.
      '';
    };
  ...
  config = mkMerge [
    {
      boot.kernelParams = mkIf cfg.setIommuPt [ "iommu=pt" ];
    }
      ...
      assertions = [
        {
          ...
          message = ''
            services.fastflowlm requires boot.kernelParams = [ "iommu=pt" ].
            Leave services.fastflowlm.setIommuPt enabled, or set the kernel
