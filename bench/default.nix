# Benchmark matrix generator - Clean declarative structure
{
  ...
  testProfiles = {
    ...
    batchSweep = sizes: {
      params = map (b: { batch = b; fa = 1; }) sizes;
    };
    
    # Test partial GPU offloading
    memoryOffload = ngls: {
      params = map (ngl: { fa = 1; inherit ngl; }) ngls;
    };
    ...
    llama2-7b = {
      ...
      benchmarks = {
        ...
        batchSizes = {
          ...
          params = map (b: { batch = b; fa = 1; }) [1 2 4 8 32 64 128 256 512];
        };
  ...
  generateModelBenchmarks = modelName: modelConfig:
    let
